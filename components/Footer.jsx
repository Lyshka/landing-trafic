...
const Footer = ({ footerData }) => {
  ...
  return (
    <footer className="bg-darkblue lg:bg-transparent lg:bg-footer lg:bg-no-repeat lg:bg-left-bottom lg:min-h-[738px] py-12 lg:pt-0 relative">
      <motion.div
      ...
      >
        ...
        >
          <div className="text-white flex-1 border-l-[10px] border-accent py-4 lg:mt-24">
            <p className="max-w-[330px] text-xl leading-[30px] ml-10">{text}</p>
          </div>

          <form className="w-full max-w-[550px] bg-white flex-1 rounded-[10px] lg:order-2 flex flex-col px-[35px] lg:px-[75px] py-[25px] lg:py-[52px] space-y-10 drop-shadow-primary">
        ...
        <motion.div variants={fadeInUp} className="py-[120px] flex flex-col lg:flex-row justify-between lg:pr-24">
          ...
          <ul className="flex flex-col items-center lg:flex-row gap-y-4 lg:gap-y-0 gap-x-[100px] text-center">
            {links.map(({ name, href }, idx) => (
              <li key={idx} className="text-white font-light hover:text-accent transition">
                <Link href={href}>
                  <a>{name}</a>
                </Link>
              </li>
